...
def main():
    ...
    pastLands, matches = runSims(land)
    print(matches)

    land = getFutureLand(pastLands, matches)
...
def getFutureLand(pastLands, matches):
    interval = matches[1] - matches[0]
    idx = (NUM_MINUTES - 1 - matches[0]) % interval + matches[0]
    return pastLands[idx]
...
def runSims(land):
    ...
    for minute in range(NUM_MINUTES):
        ...
        # Find pattern by checking land matches with past lands
        matches = findPattern(pastLands, land)

        # If land matches found, return the matches
        if matches:
            return pastLands, matches

        # Append the land structure to past Lands
        pastLands.append(land)


def findPattern(lands, land):
    matchIdx = []

    # Check past lands and see if there are any identical lands.
    for idx, pastLand in enumerate(lands):
        if land == pastLand:
            return [idx, len(lands)]
            matchIdx.append(idx)
